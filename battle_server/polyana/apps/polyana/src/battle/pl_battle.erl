...
multicast(Raw_Reply, BattlePid) ->
    case Raw_Reply of
        ...
        {multi, Reply, {Active, PlayersDict}} ->
            ...
            lists:foreach(
                fun(PlayerSrv) when PlayerSrv == Active ->
                    PlayerSrv ! {message,
                                 <<Reply/binary, "It's your turn", "\n">>};

                    (PlayerSrv) -> PlayerSrv ! {message, Reply}
                end,
                Players
            );
...
handle_call({move, PlayerPid, Direction},
            _From,
            #state{field = Field, size = Size,
                   players = Players, order = Order} = State) ->
    ...
    case Flag of
        nok ->
            {reply, {Flag, Msg, field_to_msg(Field, Size), Players2}, State};
        ...
        multi ->
            Order3 = lose_condition(Field2, Players2, Order2, Order2),
...
in_move(up, Field, {{Y, X}, Mark}, Players, [Active|_Passive] = Order) ->
    case maps:find({Y-1, X}, Field) of
        ...
        {ok,_} ->
            {single,<<"WRONG SPACE">>, Field, Players, Order};

        error ->
            {single,<<"Border">>, Field, Players, Order}
    end;
