...
%% API
-export([start_link/1]).
-export([stop/1, get_field/1, command/2, move/3, get_players/1, multicast/5]).
...
%%  Positions = lists:foldl(fun (E, Acc)-> Acc#{E => none} end, #{}, Players),
  lists:foreach(fun(PlayerSrv) -> meadow_players_srv:add_battle_pid(PlayerSrv, self()) end, Players),
  Players2 = shuffle(Players),
%%  [B,A] = Players,
...
%%  order = Players},
%%  {ok, State}.
  Positions = set_map(Players, Values, #{}),
  Field = gen_field(M, N),
  Field2 = maps:fold(fun (_K, {Position, Mark},Acc)-> maps:update(Position, Mark, Acc) end, Field, Positions),
...
multicast(Raw_Reply, Pid, Transport, Socket, State) ->
  case Raw_Reply of
    ...
    {ok, Reply} -> Transport:send(Socket, Reply),
      State;
    Reply->Transport:send(Socket, Reply),State
  end.
...
handle_call(get_field, _From, #state{field = Field, size = Size}=State) ->
  New_Field = good_field(Field, Size),
  {reply, New_Field, State};
    ...
    battle -> Order3 = lose_condition(Field2, Players2, Order2, Order2),
      case win_condition(Order3, Players2) of
        next -> State2 = State#state{field = Field2, players = Players2, order = Order3},
          ...
          {reply, {Flag, Msg2, New_Field}, State2};
...
%%  lager:info("Active player now ~p", [Passive]),
  Order.
...
%%      Active ! close_room,
      ...
      lose_condition(Field, Players, Order2, Order2)
  end.
