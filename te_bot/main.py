...
async def cmd_start(message: types.Message):
    await message.reply("Привет, с чего начнем?", reply_markup=kb.inline_kb_start)

@dp.callback_query_handler(lambda c: c.data == 'button_reg')
async def process_callback_button1(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id)
    await Form.login_reg.set()
    await bot.send_message(callback_query.from_user.id, 'Регстирация: введите логин')
...
async def process_password(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if Answ == 'response':
            time.sleep(0.2)
            await bot.send_message(message.chat.id,text = "Успешно зарегистрирован!",reply_markup=kb.inline_kb_re_auth)
        else:
            await bot.send_message(message.chat.id,text = "Что-то пошло не так, скорее всего, пользователь с таким ником уже существует, попробуйте еще раз!",reply_markup=kb.inline_kb_start)
        data.state = None
...
async def process_password_auth(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        time.sleep(0.2)
        tn.write(b'AUTH ' + Log.encode('ascii') + b' ' + Pas.encode('ascii') + b'\r\n')
        time.sleep(0.2)
        answ1 = tn.read_eager()
        if answ1 == b'SUCCESS\n':
            time.sleep(0.1)
            await message.reply("Успешно вошли!",reply_markup=kb.inline_kb_battle)
        else:
            await message.reply("Неправильный логин или пароль!",reply_markup=kb.inline_kb_start)
        data.state = None
...
async def process_callback_button_battle(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id, 'Входим в игровую комнату...')
    await bot.send_message(callback_query.from_user.id, 'Введите вашу ставку')

    @dp.message_handler(lambda message: not message.text.isdigit(), state=Form.reg_battle_bid)
    async def failed_process_bid(message: types.Message):
        return await message.reply("Ставка должна быть числом!.\nВведите вашу ставку")

    @dp.message_handler(lambda message: message.text.isdigit(),state=Form.reg_battle_bid)
    ...
    @dp.message_handler(lambda message: message.text not in ["GOLD","SILVER"],state=Form.reg_battle_currency)
    ...
    async def process_gender(message: types.Message, state: FSMContext):
        ...
        await bot.send_message(message.chat.id,"Пробуем зайти в комнату...\n Нужно размять пальцы, напишите что нибудь!",reply_markup=markup)
...
async def process_reg_battle(message: types.Message, state: FSMContext):
    ...
    tn = connect_manager.get_connect(message.chat.id)
    time.sleep(0.1)
    tn.write(b'BATTLE ' + str(Bid).encode('ascii') + b' ' + Currency.encode('ascii') + b'\r\n')
    time.sleep(0.1)
    ...
    while found_battle:
        while answ != b'':
            ...
            if b'Congratulations!' in battle_map.split():
                ...
                battle_map = battle_map.replace('-','')
                if battle_map != '':
                    if battle_map != 'NOT YOUR MOVE! WAIT PLEASE!' and battle_map != 'ERROR, please try once more':
                        await bot.send_message(id, battle_map,reply_markup=kb.inline_kb_move)
                        await Form.move.set()
...
async def check_oponent(id):
    tn = connect_manager.get_connect(id)
    battle_map = b''
    while battle_map == b'' or battle_map == b'Searching the opponent...\n' or battle_map == b'TO BATTLE!\n':
        answ = tn.read_eager()
        battle_map = answ
...
async def process_callback_button_quit(callback_query: types.CallbackQuery):
    await bot.answer_callback_query(callback_query.id)
    await bot.send_message(callback_query.from_user.id,"Ты всегда можешь вернуться, просто введи /start")
    tn = connect_manager.get_connect(callback_query.from_user.id)
    tn.close()
